...
async function mongo(req, _, next) {
  if (db === undefined) {
    try {
      const uri = config.get("database.mongodb.connection_string") ? config.get("database.mongodb.connection_string") : `mongodb://${config.get("database.mongodb.host")}:${config.get("database.mongodb.port")}`;
